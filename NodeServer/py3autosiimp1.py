...
for filename in glob.glob('iplog*'):
    ...
    with open(filename,'r')as f:
     ...
     for key,val in list(templog.items()):
       ...
       if 'tcp' in packet or 'udp' in packet:
         ...
         try:
            if packet.ip.src in userlog:
                        ...
                        #userlog[packet.ip.src]['length'] = int(packet.length)
                        userlog[packet.ip.src]['totalsize'] = int(packet.length)+int(userlog[packet.ip.src]['totalsize'])
                        userlog[packet.ip.src]['length'] = int(userlog[packet.ip.src]['totalsize']/userlog[packet.ip.src]['count'])
                        if packet.highest_layer not in userlog[packet.ip.src]['protocol']:
                            print("adding new protocol")
                            userlog[packet.ip.src]['protocol'][packet.highest_layer]=1
                        else:
                            userlog[packet.ip.src]['protocol'][packet.highest_layer]=userlog[packet.ip.src]['protocol'][packet.highest_layer]+1
                        ...
                        userlog[packet.ip.src]['totalsize']=packet.length
                        userlog[packet.ip.src]['protocol']=dict()
                        userlog[packet.ip.src]['protocol'][packet.highest_layer]=1
